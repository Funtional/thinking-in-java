...
class SList<T> {
    ...
    private class SListIteratorImpl implements SListIterator<T> {
        ...
        public void remove() {
            ...
            for (Link<T> cursor = header; ; cursor = cursor.next) {
                if (cursor.next == lastReturned) {
                    cursor.next = lastReturned.next;
                    Link<T> l = lastReturned;
                    l.next = null;
                    l.element = null; // help GC
                    break;
                }
            }
            lastReturned = header;
        ...
        public void add(T element) {
            ...
            if (header.next == next) { // Empty list
                header.next = newLink;
            } else {
